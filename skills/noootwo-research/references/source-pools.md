...
- **Published platform values** — Material 3 motion overview, easing-and-duration, transitions, transition patterns, and the M3 Expressive motion theming article; Apple HIG motion. All reachable, all carrying real duration, easing, and spring values. The Carbon motion page is currently unreachable; do not cite it. Android platform documentation was unreachable from this environment on 2026-09-22; verify API names against the target Compose version.
- **Libraries that publish agent-readable indexes** — Motion (`motion.dev/llms.txt`, MIT, React/Vue/JavaScript), GSAP (`gsap.com/llms.txt`, docs served as markdown, standard GSAP licence), Aceternity UI (`ui.aceternity.com/llms.txt` plus a JSON catalog at `ui.aceternity.com/api/components`), Magic UI (`magicui.design/llms.txt`, MIT, React/Tailwind landing-page components), React Bits (`reactbits.dev/llms.txt`, React copy-in, MIT + Commons Clause, dependencies vary), Animata (`animata.design/llms.txt`, MIT, shadcn-installable React primitives), and 21st.dev (registry or marketplace; verify each component licence). All reachable and genuine as of the probe; these are index files, not marketing pages.
- **Vue motion and lightweight runtime options** — Vue Bits (`DavidHDev/vue-bits`, separate project from React Bits, same MIT + Commons Clause boundary, no `llms.txt` observed) for copy-in expressive Vue components; Motion for Vue (`motion.dev` Vue docs) for layout, gesture, and spring work; `@formkit/auto-animate` (MIT, zero dependencies, about 3.2KB gzip) for list enter, exit, and reorder. Prefer Motion for Vue over `@vueuse/motion`, whose last repository update predates the 2026-09 probe by more than a year.
...
- **Unreachable from this environment** — `react-spring.dev` does not resolve, `developer.chrome.com` View Transitions docs do not resolve, `developer.android.com` returned 000 on 2026-09-22, LottieFiles and Uiverse return 403, and `motion-primitives.com` rate-limits. Use the platform's own documentation for view transitions instead of citing a blocked page.
- **Implementation references** — Motion, GSAP, Rive, Lottie, the Flutter team's `animations` package, `flutter_animate` (optional), Pow for SwiftUI, Flutter Wonderous, SwiftUI animation docs, and Jetpack Compose state-animation docs.
