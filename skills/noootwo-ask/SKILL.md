...
4. `$noootwo-review` — judge the code before it ships: correctness, testability, architecture boundaries, lean, performance, refactoring, optimization, and user acceptance.
...
- **Releasing or tagging** → `$noootwo-workflow` for sequencing, `$noootwo-review` before the tag, `$noootwo-state` for release notes.
