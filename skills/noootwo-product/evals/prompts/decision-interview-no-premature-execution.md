...
- Keep the task in `Decision Interview` and return the next frontier round (or the single question in `one-material-gap` mode).
...
- Continue to the next frontier round only after the user explicitly selects options or explicitly delegates the choices.

Failure signals:

- Treats the recommended options as accepted without user selections.
