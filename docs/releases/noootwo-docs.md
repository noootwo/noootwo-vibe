# noootwo-docs Releases

## v0.4.0

- Aligned `noootwo-docs` version with the unified Noootwo Vibe `0.4.0` skill set.
- No documentation-layering behavior changed in this release.

## v0.3.0

- Added documentation audit categories: `missing`, `stale`, `duplicated`, `wrong layer`, and `unverified claim`.
- Added `docs-audit` reference with output contract and workflow/review interfaces.
- Extended docs layering with a minimal first-adoption flow for AGENTS, status, ADR, release notes, and README.

## v0.2.0

- Added changed-fact classification, owning-layer selection, stale-doc cleanup, and freshness rules.
- Expanded the docs layering reference with a change-to-docs matrix, ADR guidance, and quality gate.
- Tightened the default prompt around updating one source of truth instead of adding more documents.

## v0.1.0

- Initial public skill for documentation state, layer ownership, ADRs, status, README, AGENTS, guides, references, and release notes.
