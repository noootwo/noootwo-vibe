...
The expected public skill order is `noootwo-ask`, `noootwo-workflow`, `noootwo-product`, `noootwo-design`, `noootwo-tdd`, `noootwo-review`, `noootwo-state`, `noootwo-debug`, `noootwo-research`, `noootwo-onboard`. The order is load-bearing: the validator compares the manifest against it. Do not add `noootwo-architecture` or a separate performance skill; architecture, technical judgment, tests, and performance remain `noootwo-review` lenses.
